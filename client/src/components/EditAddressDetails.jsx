    ...
    const onSubmit = async (data) => {
        ...
        try {
            const response = await Axios({
                ...
                data: {
                    ...data, // spread operator to include all the data from the form ...data is the same as writing address_line: data.address_line, city: data.city, state: data.state, pincode: data.pincode, country: data.country, mobile: data.mobile
                    address_line: data.address_line,
                    city: data.city,
                    state: data.state,
                    pincode: data.pincode,
            ...
            <section className="fixed top-0 left-0 right-0 bottom-0 bg-black bg-opacity-80 z-50 p-4">
                <div className="bg-white p-4 w-full max-w-2xl max-h-[90vh] mt-4 mx-auto overflow-y-scroll lg:custom-scrollbar-green">
                    <div className='flex justify-between items-center gap-4 bg-yellow-100 p-3 rounded border-2 border-dashed border-yellow-500'>
                        <h2 className='font-semibold kode_mono_bold text-3xl underline'>Edit Address</h2>
